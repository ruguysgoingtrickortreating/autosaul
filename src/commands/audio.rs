            ...
            Ok((connection_info, _)) => {
                ...
                if let Err(err) = lavaclient.create_player_context(guild_id, info).await {
                    match err {
                        ...
                        _ => {
                            ctx.say(format!(
                                "😿 error creating audio server connection: `{}`",
                                err
                            ))
                            .await?;
                            return Err(err.into());
...
pub async fn remove(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let queue = player.get_queue();
    let len = queue.get_count().await?;
    if len == 0 {
        ctx.say("no tracks in queue").await?;
        return Ok(());
    }
    ...
    ctx.say(format!("removed '**{title}**' from the queue"))
        .await?;
    Ok(())
}
...
pub async fn queue(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let response = queue
        .enumerate()
        .map(|(id, t)| format!("#{} - '**{}**'", id + 1, t.track.info.title))
        .collect::<Vec<String>>()
        .await
        .join("\n");
    ...
    let embed = match now_playing {
        Some(track) => {
            ...
            CreateEmbed::new()
                .title(format!("NOW PLAYING: {}", track.info.title))
                .url(track.info.uri.unwrap_or_default())
                .thumbnail(format!(
                ...
                .field(
                    ...
                    format!(
                        ...
                        format_timestamp(track.info.length)
                    ),
                    false,
                )
                .field(
                    "",
                    format!("{}", format!("[{} 🔘 {}]", pgfilled, pgempty)),
                    false,
                )
                .color(Color::RED)
                .field(
...
pub async fn rewind(ctx: Context<'_>, time: Option<String>) -> Result<(), Error> {
    ...
    match player.set_position(rewind_time).await {
        Ok(_) => {
            ctx.say(format!("rewund {} seconds", skipamnt.as_secs()))
                .await?;
        }
        Err(err) => {
            ctx.say(format!("😿 ran into an error: {}", err)).await?;
...
pub async fn fastforward(ctx: Context<'_>, time: Option<String>) -> Result<(), Error> {
    ...
    match player.set_position(fwd_time).await {
        Ok(_) => {
            ctx.say(format!("fast forwarded {} seconds", skipamnt.as_secs()))
                .await?;
        }
        Err(err) => {
            ctx.say(format!("😿 ran into an error: {}", err)).await?;
...
pub async fn set(ctx: Context<'_>, setting: String, parameter: f64) -> Result<(), Error> {
    ...
    let existing_filter = player
        .get_player()
        .await?
        .filters
        .expect("no existing filters");
    ...
    player
        .set_filters(lavaplayer::Filters {
...
pub async fn joe(ctx: Context<'_>, #[rest] biden: Option<String>) -> Result<(), Error> {
    ...
    if !biden.starts_with("biden mode") {
        return Ok(());
    }
